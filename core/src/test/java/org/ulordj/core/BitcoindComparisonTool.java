...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/ulordj
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    rulesSinceFirstFail++;
                    log.error("ERROR: bitcoind and ulordj acceptance differs on block \"" + block.ruleName + "\"");
                }
                if (block.sendOnce)
